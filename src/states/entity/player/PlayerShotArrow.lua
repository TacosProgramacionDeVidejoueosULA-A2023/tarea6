...
function PlayerShotArrowState:init(player, dungeon)
    ...
    self.bowHitbox = Hitbox(hitboxX, hitboxY, hitboxWidth, hitboxHeight)
...
function PlayerShotArrowState:update(dt)
    ...
    for k, entity in pairs(self.dungeon.currentRoom.entities) do
        if entity:collides(self.bowHitbox) then
            entity:damage(1)
...
function PlayerShotArrowState:render()
    ...
    -- love.graphics.rectangle('line', self.bowHitbox.x, self.bowHitbox.y,
    -- self.bowHitbox.width, self.bowHitbox.height)
